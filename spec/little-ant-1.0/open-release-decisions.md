...
Only unresolved semantic or security boundaries that block trustworthy Allium
or implementation remain here. Tuning values belong in
[`configuration-and-calibration.md`](configuration-and-calibration.md);
ordinary implementation choices belong in the later implementation plan.

| ID | Blocking decision | Resolution evidence |
|---|---|---|
...
| `OPEN-SKIP-001` | Complete symptom-to-reaction catalog, including Nature-sensitive applicability, the bounded `vague` clarification flow, recovery-versus-deferral outcomes, Domain scope, and exact reaction-screen grammar. | One dumb-REPL simulation for every served-work symptom, followed by powered-up and Skill parity review. |
...
| `OPEN-DEL-001` | Final Delegation lifecycle names and complete screen sequence for draft, initial preview, approval, cancellation-before-send, due follow-up, approve/decline/later, reported completion, refusal, and terminal give-up. | One `once`, `every`, and explicitly-`none` Delegation, including failed delivery and Nature-aware scope. |
| `OPEN-EFF-001` | Closed 1.0 external-effect families and their authority model, especially general write-back and notification after arbitrary completion-time `spawn` is retired. | Completion, source reconciliation, calendar write-back, delegation delivery, failure, retry, and compensation scenarios. |
...
| `OPEN-SCH-001` | Final `scheduled_commitment` required interval data, action grammar and attended/missed/cancelled outcomes, conflicts with current focus or overlapping commitments, active hard-prerequisite presentation, and delegation scope. Active commitment precedence over the ordinary draw is settled. | Flight, appointment, meeting, service-window, overlap, and time-zone-crossing simulations. |
| `OPEN-SCH-002` | Relative-preparation behavior when the commitment anchor is rescheduled, including pending versus completed children, explicit date overrides, newly overdue preparation, and whether a preview is mandatory. | Reschedule a flight and meeting before and after preparation begins. |
...
| `OPEN-UX-003` | Whether `[/] more...` appears on every non-input screen or only selected roots, and how a full command entered during a pending one-key decision preserves or cancels that decision. | Focus, comparison, confirmation, choice, nested assistance, and input-mode navigation simulations. |
| `OPEN-CLI-001` | One canonical executable name for 1.0 (`la` or `lant`) and whether the other name is retired or justified strictly as a packaging shim without becoming a core alias. | README/installation and shell-collision review. |
| `OPEN-REF-001` | Reference grammar for full opaque IDs, displayed short IDs, pasted `#shortid`, action tokens, autocomplete selection, ambiguous prefixes, and recovery. | Repeated-title, short-prefix collision, stale title, and cross-surface selection simulations. |
...
| `OPEN-DAT-002` | Authoritative-log behavior for malformed records, unknown event versions, references to missing entities, and partial corruption: fail, quarantine, or explicit degraded mode, with no silent evidence loss. | Corrupt-log, unknown-version, repair, backup, and migration rollback fixtures. |
...
| `OPEN-EXT-002` | Which listed importers, including the required official Google Calendar adapter, and UIAdapters ship inside the offline standard Pack versus the pinned companion catalog. | API feasibility and release packaging review; all listed sources remain supported 1.0 targets. |
| `OPEN-CAL-001` | Google Calendar write-back authority and approval grammar; recurring-series versus occurrence adoption; all-day-event routing; cancellation reconciliation; and calendar/attendee privacy projections. Source-to-Little-Ant observation and reconciliation are already required. | Flight, meeting, recurring swimming, all-day, reschedule, cancellation, and access-loss simulations against a fake provider. |
...
| `OPEN-MIG-001` | User-confirmed mapping for each legacy `area`, ambiguous `kind`, Party detail, and unsupported v0 relation. | Real v0 migration dry-run report. |

## Deliberately not release blockers

The following do not justify inventing more core behavior before simulation:
