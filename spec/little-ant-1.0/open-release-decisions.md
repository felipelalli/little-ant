...
| `OPEN-FOC-001` | Exhaustive versioned 1.0 focus-opportunity variant catalog and the actions valid for each variant. | Coverage matrix across the synthetic week and v0 regression audit. |
...
| `OPEN-WRK-001` | Final public commands for resuming focus, WIP-to-idle, standing-run finish, and retiring standing work. Transactional `/next` browsing and non-state `/pause` are settled by `FOC-033`, `WRK-048`, and `UX-066..067`. | Focus, interruption, checklist, and repeatable-work screens. |
...
| `OPEN-UX-001` | Remaining shortcut collisions after settling `[b]locked`, `bo[r]ed`, and `[d]one`; choice/input layouts, contextual-assistance labels, accessible no-emoji rendering, and the exact English copy for all four settled 16-entry personality catalogs. | Almost-literal REPL/web/Skill screen and factory-copy review. |
...
| `OPEN-UX-003` | Which non-input grammars beyond Focus expose `[/] more...`, plus exact recovery for empty searches, unavailable commands, ambiguous selection, and guided arguments. Focus already suspends its pending interaction and revalidates it after mutation under `UX-022`, `UX-047`, and `UX-M01`. | Comparison, confirmation, choice, nested assistance, empty-search, unavailable-command, and guided-argument simulations. |
