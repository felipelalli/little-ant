...
| `OPEN-UX-003` | Which remaining non-input grammars expose `[/] more...`, plus exact recovery for empty searches, unavailable commands, ambiguous selection, and guided arguments. Focus and the served-work symptom screen already expose it; Focus suspends its pending interaction and revalidates it after mutation under `UX-022`, `UX-047`, and `UX-M01`. | Comparison, confirmation, choice, nested assistance, empty-search, unavailable-command, and guided-argument simulations. |
