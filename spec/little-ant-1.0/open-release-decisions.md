...
| `OPEN-WAIT-001` | `review_not_before`, weighted eligibility after opening, no due/overdue semantics, the human-response dumb choices and three-day factory suggestion, bounded history/assistance, early source proposal, request handoff, and outcome families are settled by `FOC-034` and `WRK-050..055`. Remaining work: exact custom date/time selector, pressure curve and evidence threshold, non-response escalation cap, source-resolution confirmation, and follow-up cadence. | Short/long human response, custom instant, source-observed response, repeated no-response, follow-up, reclassification, and resolution simulations. |
