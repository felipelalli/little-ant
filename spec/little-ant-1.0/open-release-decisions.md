...
| `OPEN-FOC-001` | Semantic granularity is settled by `FOC-009`: distinct action/transition families require discriminated variants; an identical family may use typed `purpose`; signals, warnings, and context are not opportunities; and screen grammar is orthogonal. Remaining work: the exhaustive versioned 1.0 variant names, required payloads, purposes, actions, and transitions. | Coverage matrix across the synthetic week and v0 regression audit, including rejection of hybrid payloads and accidental signal-as-opportunity tickets. |
