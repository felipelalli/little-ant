...
| `other` requires raw text and repeated `other` reports trigger taxonomy review. | **partly retained / review** | Raw evidence survives. Decide whether repeated `other` should create a derived taxonomy-review opportunity and how its configurable threshold works. |
...
| Delegation has draft/notice approval, cancellation before notice, due follow-ups, approve/decline nudge, and terminal completed/refused/abandoned outcomes. | **partly retained / review** | Current Allium retains several statuses but not the complete follow-up state machine. Review the v0 behavior explicitly before naming commands. |
...
| `unify` retargets relationships and removes would-be self-comparisons. | **review** | Duplicate reuse/enrichment does not yet specify a general Brick-to-Brick merge. Decide whether explicit merge survives and define every relationship, history, and evidence consequence. |
...
| Tree, one-line, table, CSV, Org, and self-contained HTML projections exist. | **review** | Do not silently drop these useful views. Decide which remain first-party projections, which become ReadOnlyExporters or UIAdapters, and which are outside 1.0. |
...
| Every command lazily fires due temporal rules; `tick` can run them explicitly. | **review** | The REPL has idle/safe-boundary timing and core rules are deterministic, but CLI tick cadence, idempotence, and explicit administration need confirmation. |
