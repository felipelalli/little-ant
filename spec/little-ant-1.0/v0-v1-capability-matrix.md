...
| Explicit start, stop, and direct done preserve work history | `I+T` | **strengthened** | `WRK-001..006`, `WRK-048..049`, `WRK-065`, `WRK-139..140`, and UX-F06/F10/F15 define focused, WIP, idle, done, finish, and retirement boundaries without aliases | focus/WIP interruption route |
...
| Kind or behavior can be edited as loosely coupled fields | `I+T` | **replaced and constrained** | behavior means exactly one Nature; timing and plan retain their own routes; Template is immutable creation provenance; the closed capability profiles derive all 81 transitions without silent conversion under `MOD-058..064`, `MOD-077..082`, `WRK-106..107`, `WRK-112`, `WRK-116`, `WRK-122`, UX-S46..S47, UX-NAT01, and UX-S57..S59 | generated transition matrix and paired behavior-update replay |
| Closing a parent silently re-roots active children | `I+T` | **retired** | explicit parent-only or subtree scope, visible reparenting, mixed terminal-child review, and gate reconciliation under `MOD-015`, `MOD-083..084`, `WRK-129..130`, and UX-LC00/LC04/LC05 | project closure route |
...
| Completion effects `write_back`, `notify`, and arbitrary `spawn` stop for approval | `I+T` | **deliberately narrowed** | external preview retained by `PRD-009`; DAT-068 closes the seven supported purposes; generic write-back, notification, and arbitrary spawn are retired | `SCN-EFF-001` |
