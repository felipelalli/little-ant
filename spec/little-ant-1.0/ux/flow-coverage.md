...
```text
specified — canonical semantics, dumb screens, and an implementation-acceptance route exist
verified  — a conforming implementation has passed the cited deterministic replay
```

Every 1.0 route below is semantically `specified`. None is claimed
`verified` before implementation; the last column is test evidence to produce,
not missing product design.

| Gate | Flow | Canonical references | Specification | Required implementation evidence |
|---:|---|---|---|---|
...
| 2 | Revision/stale response | `UX-029..031`, `UX-191..193`, UX-ER02, `SCN-REC-001` | specified | concurrency, crash, and integrity-token fixture replay |
| 2 | Reference selection | `MOD-010`, `MOD-024`, `UX-075..076`, `UX-201..203`, UX-RF01, UX-RF04 | specified | edge-normalization and deterministic cross-surface replay |
| 3 | Restore/startup to `next` | `UX-R00`, `UX-F04..F05`, `UX-F09`, `UX-046`, `UX-063`, `UX-080` | specified | pending-envelope result transition |
| 3 | Pristine first start | `UX-E00`, `FOC-029` | specified | Feed/back/end-to-end transition |
| 3 | Useful non-pristine empty | `UX-E01`, `FOC-028`, `SCN-EMPTY-001` | specified | state-derived choice transitions |
| 3 | Feed text input | `FED-049`, `UX-M01`, `UX-I02`, `UX-047..049`, `UX-144`, `UX-184`, `SCN-FED-002` | specified | deterministic post-commit revalidation, pristine transition, empty-input failure, paired powered-up proposal, and crash replay |
| 3 | Preserve/normalize Raw | `MOD-001`, `MOD-029`, `MOD-049`, `MOD-056..057`, `MOD-065..072`, `FED-052..056`, `WRK-105`, `UX-158..162`, `UX-186..190`, `DAT-046..050`, `UX-S42..S45`, `UX-RA00..RA04` | specified | deterministic paired internal/external-editor and dumb/assisted transcript replay |
...
| 3 | Raw triage and disposition | `MOD-001`, `MOD-054`, `MOD-065..072`, `FED-001..009`, `FED-042..056`, `FOC-051..052`, `UX-144..150`, `UX-184..190`, `UX-T01..T10`, `UX-RA00..RA04` | specified | deterministic triage, shelf/link/source, archive, revision, and source-recovery replay |
| 3 | Duplicate suspicion | `MOD-008..012`, `MOD-063`, `MOD-067`, `FED-015..018`, `FED-046`, `FED-051`, `FOC-054`, `UX-149`, `UX-285`, `UX-T05`, `UX-T08`, `UX-T10` | specified | repeated Feed through Raw grouping, existing Work/ListEntry reuse, preserved receipts, and durable Brick-to-Brick merge outcomes plus deterministic replay |
...
| 3 | Focus proposal and active completion | `UX-F01..F05`, `UX-F07`, `UX-F12..F14`, `UX-M01`, `FOC-048..050`, `WRK-002`, `WRK-005`, `WRK-049`, `UX-069`, `UX-139..143`, `SCN-FOC-006` | specified | nested UX-F13 uncertainty tree, exact `/show` projection, and replay through ordinary and zero-eligible post-completion `next` results |
| 4 | Ordering uncertainty, skip, and provisional placement | `IMP-008..010`, `IMP-040..046`, `UX-096`, `UX-134..138`, `UX-O05`, `UX-O11..O15`, `SCN-IMP-002`, `SCN-IMP-004` | specified | deterministic end-to-end replay including `/tie-break` and no-nearby-alternative placement |
...
| 4 | Adaptive bulk ordering | `IMP-005`, `IMP-031..044`, `FOC-041..043`, `UX-091..102`, `UX-134..138`, `UX-O01..O15` | specified | deterministic end-to-end replay |
...
| 4 | Served-work symptom | `UX-S01..S47`, UX-S08E, UX-S15A, UX-S35A, UX-S37A, UX-DT00..DT02, UX-UP00..UP02, `WRK-007..013`, `WRK-047`, `WRK-067..128`, `UX-084`, `UX-103..133`, `UX-152..164`, `UX-211..217`, `FED-030..041`, `IMP-039`, `FOC-047`, `DAT-045`, `SCN-WRK-002`, `SCN-WRK-005` | specified | deterministic replay of every symptom, Nature-owned completion/deferral, shared date input, and paired assisted surface; downstream Feed or handoff results remain with their owning flows |
...
| 4 | Meaning, title, and attached description | `MOD-056..057`, `MOD-065..071`, `FED-037`, `FED-052..055`, `WRK-103..105`, `UX-156..162`, `UX-186..188`, `DAT-046`, `UX-S39..S45`, `UX-RA00..RA03`, `SCN-ARC-001` | specified | remaining uncertainty trees and paired surface replay |
...
| 4 | Global search and return | `UX-151`, `UX-RF03`, `SCN-SRCH-001` | specified | deterministic cross-kind ranking, pagination, and paired surface replay |
| 4 | Global history and recent recap | `DAT-052..054`, `UX-027..028`, `UX-151`, `SCN-HIS-001` | specified | bounded recap, filtered event history, pagination, sparse projection, and exact return replay |
| 5 | Cross-Domain focus | `MOD-073..076`, `FED-057..058`, `UX-F02`, `UX-F12..F14`, `UX-DM00..DM03`, `FOC-014..019`, `FOC-055..060`, `UX-205..210`, `SCN-DOM-001`, `SCN-FOC-001`, `SCN-FOC-002` | specified | deterministic forest, hard-scope, continuity, positive tail, tie-path, and edge-case replay |
...
| 5 | Wait activation and review | `MOD-050`, `FOC-034`, `WRK-050..055`, `WRK-127`, `WRK-134..138`, `UX-235..237`, `UX-S02C`, UX-W00..W03, UX-DT00..DT02, `SCN-WAIT-001`, `SCN-WRK-003` | specified | activation, gating, pressure, history-backed timing, follow-up strategy, source confirmation, typed skip, and paired assisted replay |
...
| 5 | Lazy claim review lottery | `MOD-052..053`, `IMP-030..031`, `FOC-040..041`, `UX-090..091`, `UX-095`, `UX-B02`, `UX-N01`, `UX-O01`, `UX-O04`, `SCN-FOC-004` | specified | exact Nature-review result envelope and deterministic paired replay |
| 5 | Importance/forecast projections | `IMP-003`, `IMP-058..063`, `FOC-002`, `FOC-013`, `FOC-054..070`, `UX-210`, `UX-280`, UX-LIST00, `MOD-076`, [`deterministic-calculation-profile.md`](../deterministic-calculation-profile.md), `SCN-FOC-005`, `SCN-FOC-007` | specified | fixed-point reference vectors, provenance decay, semantic opportunity granularity, fixed-stream probabilities, opportunity-catalog fixtures, pagination, scope, and recursive query replay |
| 5 | Required/preferred Place context | `WRK-035..037`, `WRK-157..160`, `FOC-063`, `UX-276..277`, UX-PL00..PL01, `SCN-PLACE-001` | specified | value-only labels, observation validity, selected-Work continuation, cooldown, privacy, direct maintenance, and paired assistance |
| 6 | Living-checklist run | `UX-L00..L01`, `MOD-003`, `MOD-035..037`, `MOD-063`, `WRK-019..020`, `WRK-117..118`, `UX-180..181`, `SCN-LST-001` | specified | deterministic transcript replay and exact undo compensation |
| 6 | Finite-checklist run | `UX-083`, `UX-L00..L01`, `MOD-003`, `MOD-036..037`, `MOD-063`, `FOC-039`, `WRK-066`, `WRK-117..118`, `UX-180..181`, `SCN-LST-001` | specified | deterministic transcript replay and exact undo compensation |
| 6 | Nature capability matrix | `MOD-025..047`, `MOD-062..064`, `MOD-077..082`, factory Nature tables | specified | generated all-Nature capability and 81-transition conformance matrix |
...
| 8 | Raw review/source reconcile | `MOD-065..072`, `FED-010..014`, `FED-052..056`, `UX-186..190`, `DAT-011..017`, `DAT-047..050`, UX-RA00..RA04 | specified | deterministic end-to-end and corrupt-blob fixture replay |
| 8 | `/translate` | `MOD-049`, `MOD-071`, `FED-054..055`, `UX-188`, UX-RA03 | specified | deterministic dumb/assisted queue replay |
...
| 8 | Profiles, preferences, and credential vault | `PRD-011`, `MOD-090..092`, `DAT-024..027`, `DAT-057..067`, `DAT-093`, `UX-260..264`, `UX-283`, UX-CNT00..CNT01, UX-VLT00, UX-CFG00, `SCN-SEC-001` | specified | separate typed XDG stores, safe profile creation, multi-profile resolution, age-v1 vault, recovery/rotation, memory agent, locked-state return, and redaction threat replay |
...
| 8 | Pack trust/install/update/remove | `DAT-018..023`, `DAT-033`, `DAT-086..091`, `UX-270..272`, `UX-286`, UX-PACK00..PACK04, `SCN-PACK-001`, [`pack-format-and-trust.md`](../pack-format-and-trust.md) | specified | reproducible archive, signatures, official/community trust, revocation, zero cross-Pack dependencies, explicit updates, safe preferred-pin removal, and global archive collection |
| 8 | Canonical commands and safe export | `UX-022..024`, `UX-074`, `UX-194..195`, `UX-204`, `UX-278..284`, `DAT-028..030`, `DAT-092`, UX-LIST00, UX-EXP00, [`command-catalog.md`](../command-catalog.md), `SCN-CMD-001` | specified | registry enumeration, palette subset, unknown vocabulary, exact CLI mapping, tick equivalence, list projections, and export destination safety |
| 8 | Typed reference/error/dry-run | `MOD-008..010`, `UX-040..042`, `UX-075..076`, `UX-RF01..RF02`, `SCN-REF-001` | specified | technical-reference, not-found, precondition, edge-normalization, and generic dry-run screens |
| 8 | Corrupt history/repair | `DAT-049`, `DAT-055..056`, `MIG-001..044`, UX-ER01, `SCN-DAT-001` | specified | sparse projection, corrupt-log, unknown-version, backup, and repair fixtures |
...
| 9 | Powered-up paired replay | `UX-033..039`, `UX-043`, `UX-059..060`, `UX-086..087`, `UX-B00A..B00B` | specified | replay accepted dumb flows |
| 10 | Skill and local-web mirror | `UX-002`, `UX-039`, `UX-044`, `UX-269`, screen-catalog surface mapping, `SCN-FED-004`, `SCN-UIA-001` | specified | paired dumb/powered-up/Skill/local-web replay; future mobile must pass the same conformance contract but is not a 1.0 deliverable |

An implementation marks a route `verified` only when its replay records:
