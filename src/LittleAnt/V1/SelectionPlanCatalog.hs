...
import LittleAnt.V1.Domain
  (Authority (Human), Brick (..), BrickBehavior, BrickId,
   BrickStatus (..), DomainError, DomainState (..), WorkState (..),
...
import LittleAnt.V1.Material
  (MaterialError, Raw (..), ReviewDispositionKind (Retained), archiveRaw,
   captureInlineRaw, emptyMaterialState, registerMaterialBrick, reviewRaw)
import qualified LittleAnt.V1.Priority as Priority
import LittleAnt.V1.Selection
import LittleAnt.V1.Standing
  (StandingError, StandingState (..), createStandingBrick, emptyStandingState,
...
proposalRegistrations = concat
  [ successAndCreation "DeferredPlacementCreatesPriorityProbe"
  , successAndCreation "JudgmentProbeCreatesAxisProposal"
  , successAndCreation "ReopenedJudgmentProbeCreatesAxisProposal"
  , successOnly "DeferredJudgmentProbeResolvesAxisProposal"
  , successAndCreation "ParentReviewProposed"
  , successAndCreation "WipReviewProposed"
  , successAndCreation "ScopeReviewProposed"
  , successAndCreation "DependencyReviewProposed"
  , successAndCreation "NewRawCreatesReviewProposal"
  , successOnly "ReviewedRawResolvesReviewProposal"
  , successOnly "ArchivedRawResolvesMaterialProposals"
  , successOnly "ProposalResolved"
  , successOnly "ProposalDismissed"
  , successAndCreation "RepeatedPracticeFrictionCreatesReview"
  , successAndCreation "StaleFocusProposed"
  ]
  where
    successOnly construct =
      [selectionRegistration "rule_success" construct proposalDerivationProbe]
    successAndCreation construct =
      [selectionRegistration category construct proposalDerivationProbe
      | category <- ["rule_success", "rule_entity_creation"]]
...
forecastRegistrations =
  ...
  , selectionRegistration "rule_success" "ValidCurrentFocusReturnedByNext" forecastProbe
  , selectionRegistration "rule_entity_creation" "ValidCurrentFocusReturnedByNext" forecastProbe
  , selectionRegistration "rule_success" "OrdinaryNextDrawnFromForecast" forecastProbe
  , selectionRegistration "rule_entity_creation" "OrdinaryNextDrawnFromForecast" forecastProbe
  ]
...
deferredPlacementProbe = do
  ...
  require (any (\proposal -> proposalKind proposal == PriorityProbe
      ...
      created)
    "deferred placement did not derive a priority_probe"
...
judgmentProposalProbe = do
  ...
  (_, resolved) <- selection (advanceSelection probeTime deferredContext withProposal)
  ...
  require (proposalStatus retained == ProposalResolved)
    "deferred JudgmentProbe did not resolve its open axis proposal"
...
staleProposalProbe = do
  ...
  (createdAgain, _) <- selection (advanceSelection staleAt focusedContext first)
  require (null createdAgain)
