...
packRoot :: FilePath
packRoot = "packs/official-connectors"

payloadRoot :: FilePath
payloadRoot = packRoot </> "payload"

archivePath :: FilePath
archivePath = packRoot </> "official-connectors.lantpack"
...
main =
  getArgs >>= \case
    ...
    _ -> die "usage: cabal exec runghc -- -isrc tools/rebuild-official-connectors-pack.hs refresh-manifest|new-signature|archive|verify"
...
createSignature = do
  ...
  putStrLn ("created one-off connector signature: " <> Text.unpack (packSignatureKeyFingerprint document))
...
createArchive = do
  ...
  ByteString.writeFile archivePath archive
...
verifyArchive = do
  ...
  actual <- ByteString.readFile archivePath
  if expected /= actual
    then die "official-connectors.lantpack is not the canonical archive of the committed source tree"
    else do
